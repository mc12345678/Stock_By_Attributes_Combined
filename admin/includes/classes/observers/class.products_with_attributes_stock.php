...
//    $attachNotifier[] = 'NOTIFY_ORDER_QUERY_ADD_PRODUCT';
...
/*    if ( $_POST['copy_sba_attributes']=='copy_sba_attributes_yes' and $_POST['copy_as'] == 'duplicate' ) {
  ...
  function notify_order_instantiate(&$callingClass, $notifier, $emptyArray, &$order_id) {
    global $current_page_base;
    if (!in_array(basename($GLOBALS['PHP_SELF'], '.php'), array(FILENAME_INVOICE))) {
      return;
    }
    if (is_null($order_id)) {
      return;
    }
    // Trigger the need to gather information about the customid.
    $this->addCustomIdToAttributes = true;
    
    // Intention is to load the language file one time while using/processing this notifier in support of later use.
    include_once DIR_WS_LANGUAGES . $_SESSION['language'] . '/' . 'products_with_attributes_stock.php';    
  }
  
  // NOTIFY_ORDER_QUERY_ADD_PRODUCT
  function notify_order_query_add_product(&$callingClass, $notifier, $productsI, &$index) {
    global $products_with_attributes_stock_class;
    
    // Early escape if not supposed to add customid to the attributes name (admin/invoice.php is the current target).
    if (empty($this->addCustomIdToAttributes)) {
      return;
    }
    
    // Early escape if there are no attributes on this product.
    if (empty($productsI['attributes'])) {
      return;
    }
    
    // Early escape if not displaying customid.
    if (!(defined('STOCK_SBA_DISPLAY_CUSTOMID') && STOCK_SBA_DISPLAY_CUSTOMID == 'true')) {
      return;
    }
    
    // This operation currently appears to only collect the individual customid from the variant list and not the customid for a variant having multiple attributes.
    foreach ($productsI['attributes'] as $subindex => $values) {
      $attributes = array();
      //create array for use in zen_get_customid
      $attributes[] = $productsI['attributes'][$subindex]['value_id'];//value_id option_id
      //get custom ID
      $customid = ' ' . $products_with_attributes_stock_class->zen_get_customid($productsI['id'], $attributes) . ' ';
      //only display custom ID if exists
      if (empty($customid)) {
        continue;
      }
      ...
      $callingClass->products[$index]['attributes'][$subindex]['value'] .= /*$attribute_name .=*/ ' (' . $customid . ') ';
    }
    
    // Would appear that to "add" the customid to the last attribute supporting a single customid for all options, that either would
    //  accumulate all of the attributes in an array and then at the last attribute when all are together to add the customid to that last 
    //  attribute, or to "double" work the determination possibly and add at the end or to some other variable after all have been processed.
  }
  ...
  function updateOrderQueryAdminComplete(&$orderClass, $notifier, $paramsArray) {
    ...
    */
    ...
    // Loop through each product in the order
      ...
      if (!empty($product) && is_array($product) && array_key_exists('attributes', $product) && !empty($product['attributes']) && is_array($product['attributes'])) {
        $orders_products_sba_customid = $db->Execute("select 
                           ...
                           WHERE  
                             ...
                             AND opas.orders_products_id = " . (int)/*$orders_products->fields['orders_products_id']*/ $product['id'] . " 
        ...
        if ($custom_multi == 'single' && zen_not_null($customid_txt)) {
      ...
      $product['customid'] = array('type' => $custom_multi, 
                                   'value' => $customid_txt,
                                  );
